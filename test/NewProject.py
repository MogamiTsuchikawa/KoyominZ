...
class NewProject(wx.Frame):
    def __init__(self,parent):
        ...
        self.ProjNameTextbox = wx.TextCtrl(self.panel,-1,pos=(100,100))
        self.ProjPathTextbox = wx.TextCtrl(self.panel,-1,pos=(100,200))
    def MakeProjBtn_Clicked(self,event):
        if self.ProjNameTextbox.GetValue != "":
            #Pathはあとでやる
            f_path = os.path.dirname(__file__)
            self.MakeNewProject(f_path,self.ProjNameTextbox.GetValue)
        
    def MakeNewProject(self,f_path,name):
        proj_path = f_path + os.pathsep + name
        print(proj_path)
